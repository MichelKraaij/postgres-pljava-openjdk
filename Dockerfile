...
RUN apt-get update && \
    ...
    java -jar $(ls /pljava/pljava-packaging/target/pljava-pg12*-amd64-Linux-gpp.jar) && \
    cd ../ && \
    apt-get -y remove --purge --auto-remove git ca-certificates g++ maven postgresql-server-dev-12 libpq-dev libecpg-dev openjdk-8-jdk libkrb5-dev libssl-dev && \
